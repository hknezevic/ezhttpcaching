...
menuArray['ClassMenu']['elements']['class-view-cache-delete'] = {ldelim} 'url': {"%currentURL%"|ezurl} {rdelim};
...
-->
    ...
    <a id="class-view-cache-delete" href="#" onmouseover="ezpopmenu_mouseOver( 'ClassMenu' )" data-class-identifier="{$module_result.content_info.class_identifier}" data-class-id="{$module_result.content_info.class_id}">{"Delete class view cache"|i18n("design/admin/popupmenu")}</a>
...
{literal}
<script type="text/javascript">
    $('a#class-view-cache-delete').on('click', function(e){
        e.preventDefault();
        var classId = $(this).data('class-id');
        var classIdentifier = $(this).data('class-identifier');
        $.ajax({
            type: 'POST',
            url: '{/literal}{symfony_url('websc.http_cache.clear_for_content_type')}{literal}',
            data: {
                'contentTypeId': classId
            },
            success: function(data){
                console.log(data.status);
            }
        });
        alert('Cleared view cache for class \'' + classIdentifier + '\'.' );
        return false;
    });
</script>
{/literal}